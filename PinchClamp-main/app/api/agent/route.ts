...
const WORKER_MODEL = process.env.AGENT_WORKER_MODEL || "qwen/qwen3.6-27b";
...
// ─── Tool Execution: scrape_website (Jina Reader → 27B Worker Summarizer) ──────
...
async function executeScrapeWebsite(url: string, groq: Groq): Promise<string> {
    ...
    // Step 2: Guardrail 1 — 27B Worker Node summarization
    // Truncate raw content to 10k chars to stay within 27B context window (approx 5.5k tokens)
    ...
    try {
        const workerResponse = await groq.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: `You are an expert business analyst AI. Read the following scraped website markdown and extract a rich, comprehensive breakdown of the company's competitive positioning.

You MUST extract and detail:
- **Core Features/Offerings**: Describe their specific products, technologies, and unique value propositions in detail. Explain *how* their platform works.
- **Target Audience**: Who is this built for? Detail their ideal customer profile (e.g., company size, industry, specific roles).
- **Pricing & Tiers**: Extract any pricing models, exact numbers, plan names, and billing cycles. If none exist, output "Pricing: Not publicly listed".
- **Marketing & Positioning**: What is their main marketing angle or claim to fame?

Write a comprehensive, highly-detailed summary. Feel free to use paragraphs and descriptive bullet points. Your goal is to give the overarching AI orchestrator the best possible context to write a professional executive brief.`,
                },
                {
                    role: "user",
            ...
            async start(controller) {
                try {
                    ...
                    while (actionCount < MAX_ACTIONS) {
                        ...
                        for (let attempt = 0; attempt < 3; attempt++) {
                            try {
                                ...
                                if (isToolUseFailed && attempt < 2) {
                                    sendStatus({ step: "thinking", message: `Retrying (fixing malformed output, attempt ${attempt + 2}/3)...` });
                                    // Inject a coaching message to fix the model's behavior
                                    messages.push({
                                        role: "user",
                                        content: "Your previous response resulted in a `tool_use_failed` error because the JSON formatting was malformed. You MUST use the proper JSON tool calling format provided by the API without any raw text. Call exactly ONE tool now.",
                                    });
                                    continue;
                                }
                        ...
                        // ─── Llama 3 Fallback: Parse raw JSON tool calls in content ───
                        if (
                            assistantMessage.content &&
                            (!assistantMessage.tool_calls || assistantMessage.tool_calls.length === 0)
                        ) {
                            const contentStr = assistantMessage.content.trim();
                            if (contentStr.startsWith("{") && contentStr.includes('"name"')) {
                                try {
                                    ...
                                    if (parsed.type === "function" || parsed.name) {
                                        ...
                                        assistantMessage.content = ""; // Clear content so it acts purely as a tool call
                                    }
                                } catch {
                        ...
                        for (const toolCall of assistantMessage.tool_calls) {
                            ...
                            if (fnName === "search_web") {
                                ...
                                sendStatus({
                                    step: "worker",
                                    message: `Summarizing ${new URL(url).hostname} via 27B worker...`,
                                });
                    ...
                    // Increment usage counter (skip for localhost)
                    if (!isDev) {
